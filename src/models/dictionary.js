...
export default {
  ...
  effects: {
    ...
    *handleDisable({ payload }, { call, put }) {
      const response = yield call(dictionaryService.handelDisable, payload);
      if (response.status === 'OK') {
    ...
    *handleEnable({ payload }, { call, put }) {
      const response = yield call(dictionaryService.handleEnbale, payload);
      if (response.status === 'OK') {
