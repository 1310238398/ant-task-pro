...
class dictionaryList extends PureComponent {
  ...
  handleDisableClick() {
    ...
    const item = selectedRows[0];
    Modal.confirm({
      title: `确定禁用【字典数据：${item.name}】？`,
      okText: '确认',
      okType: 'danger',
      cancelText: '取消',
      onOk: this.disOKClick.bind(this, item.record_id),
    });
  }

  disOKClick(id) {
    this.dispatch({
      type: 'dictionary/handleDisable',
      payload: id,
    });
    this.clearSelectRows();
  }
  ...
  handleUndisableClick() {
    ...
    const item = selectedRows[0];
    Modal.confirm({
      title: `确定启用【字典数据：${item.name}】？`,
      okText: '确认',
      okType: 'danger',
      cancelText: '取消',
      onOk: this.enableOKClick.bind(this, item.record_id),
    });
  }

  enableOKClick(id) {
    this.dispatch({
      type: 'dictionary/handleEnable',
      payload: id,
    });
    this.clearSelectRows();
  }
                  ...
                  {selectedRowKeys.length === 1 && [
                    ...
                    selectedRows[0].status === 2 && (
                      <PButton
                        key="undisable"
                        code="undisable"
                        onClick={() => this.handleUndisableClick()}
                      >
                        启用
                      </PButton>
                    ),
